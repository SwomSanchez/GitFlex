...
PROFILE_SCRIPT_TEMPLATE = '''"""
generate_profile.py - Auto-generated by GitFlex Engine
Runs autonomously in GitHub Actions and generates assets/profile.svg
"""
...
USERNAME = os.environ.get("GITHUB_USERNAME", "__USERNAME__")
TOKEN = os.environ.get("GH_TOKEN")
HEADERS = {"Authorization": f"token {TOKEN}"} if TOKEN else {}

CUSTOM_NAME = "__NAME__"
CUSTOM_BIO = "__BIO__"
SELECTED_TECHS = __TECH_ITEMS_JSON__

DEVICON = "https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons"
TECH_ICONS = [
    (t.capitalize(), f"{DEVICON}/{t}/{t}-original.svg") for t in SELECTED_TECHS
]
...
def get_as_base64(url, headers=None):
    ...
    try:
        ...
        if not ct:
            ct = "image/svg+xml" if url.endswith(".svg") else "image/png"
        else:
            ct = ct.split(";")[0]
        enc = base64.b64encode(r.content).decode("utf-8")
        uri = f"data:{ct};base64,{enc}"
        _ICON_CACHE[url] = uri
        return uri
...
def build_svg(user, repos, lang_stats):
    ...
    name = html.escape(CUSTOM_NAME or user.get("name") or USERNAME)
    bio = html.escape(CUSTOM_BIO or user.get("bio") or "")
    ...
    # Generate 32 Floating Background Particles
    ...
    y_coords = [530, 580]
    for idx, (tech_name, url) in enumerate(TECH_ICONS[:8]):
        row = idx // 4
        col = idx % 4
        x = x_coords[col]
        y = y_coords[row]
    ...
    for i, (lang, pct) in enumerate(lang_stats):
        ...
        bar_width = max(pct * 3.3, 8)
        pct_formatted = f"{pct:.1f}"
        bars_svg += f"""
        <g transform="translate(460, {by})">
          <text x="0" y="15" class="card-desc" font-weight="600">{escaped_lang}</text>
          <text x="330" y="15" class="card-desc" font-weight="700" fill="{colors[i % len(colors)]}">{pct_formatted}%</text>
...
def main():
    ...
    try:
        ...
        print("API error, using cached data if available:", e)
    ...
    with open("assets/profile.svg", "w", encoding="utf-8") as f:
        f.write(svg)
    print("assets/profile.svg successfully generated!")
...
class ScriptSynthesizer:
    """
    Synthesizes a standalone, zero-dependency 'generate_profile.py'
    matching 100% the exact original profile engine.
    """

    @staticmethod
    def generate_standalone_script(config: dict) -> str:
        ...
        for w in config.get("widgets", []):
            if w.get("id") == "tech_stack" and "items" in w:
                tech_items = w["items"]
                break

        tech_items_json = json.dumps(tech_items)

        code = (
            PROFILE_SCRIPT_TEMPLATE.replace("__USERNAME__", username)
            .replace("__NAME__", name)
            .replace("__BIO__", bio)
            .replace("__TECH_ITEMS_JSON__", tech_items_json)
        )
        return code
